 		...
 		$datosJson = '{
			...
			"data": [ ';
				...
				for($i = 0; $i < count($banner); $i++){
					...
					$acciones = "<div class='btn-group'><button class='btn btn-warning btnEditarBanner' idBanner='".$banner[$i]["id"]."' data-toggle='modal' data-target='#modalEditarBanner' title='Editar'><i class='fa fa-pencil'></i></button><button class='btn btn-danger btnEliminarBanner' idBanner='".$banner[$i]["id"]."' imgBanner='".$banner[$i]["img"]."' title='Eliminar'><i class='fa fa-times'></i></button></div>";
